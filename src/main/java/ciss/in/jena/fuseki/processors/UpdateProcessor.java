...
public class UpdateProcessor implements Processor {
    ...
    public void process(final Exchange exchange) throws IOException {
    	...
    	String proc = (String) exchange.getIn().getHeader("proc");
    	if (proc.equals("load"))
    		uploadProduct(exchange);
    	else if (proc.equals("update"))
    		updateProduct(exchange);
    }
    
    public void uploadProduct(Exchange exchange) {
    	        
        exchange.getOut().setHeader("Content-Type", "application/rdf+xml");
    	String loadString = (String) exchange.getIn().getBody();
        exchange.getOut().setBody(loadString);
    }
    
    public void updateProduct(Exchange exchange) {
    	String queryString = (String) exchange.getIn().getBody();
    	exchange.getOut().setHeader("update", queryString);        
