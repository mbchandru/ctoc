...
public class TDBTransactions {
	Dataset dataset;
	InputStream in;
	
	public TDBTransactions() {
	}
	
	public void initializeTDB(String NTriplesFile) {
	    FileManager fm = FileManager.get();
	    fm.addLocatorClassLoader(TDBTransactions.class.getClassLoader());
	    in = fm.open(NTriplesFile); //"RDFStore/foaf.rdf"

	    Location location = Location.create ("target/TDB");
	    dataset = TDBFactory.createDataset(location);
	}
	
	public void initializeRDF(String RDFDirectory) {
		
		dataset = TDBFactory.createDataset(RDFDirectory) ;
	}
	...
	public void readRDF(String RDFFile) {
       ...
       //Use FileManager to read the file and add it to the Jena model
        FileManager.get().readModel(model, RDFFile); //"E:/RDFStore/foaf.rdf"
